  ...
  event.forEachRecipe({ type: 'immersiveengineering:alloy' }, recipe => {
    ...
    inputs.forEach(input => {
      for (let i = 0; i < input["count"]; i++){
        finalrecipe["ingredients"].push("tag" in input ? {"tag": input["tag"]} : {"item": input["item"]})
      }
    })
    
    finalrecipe["results"].push({
      "count": result["count"],
      "item": {"id": "item" in result ? result["item"] : AlmostUnified.getTagTargetItem(result["tag"]).id}
    })
